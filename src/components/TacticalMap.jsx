...
export default function TacticalMap({
  ...
  const [activeFilter, setActiveFilter] = useState(
    recommendedAssetIds && recommendedAssetIds.length > 0 ? 'proposals' : 'all'
  );

  // When new recommendations are generated by the agent, auto-switch filter to proposals!
  useEffect(() => {
    if (recommendedAssetIds && recommendedAssetIds.length > 0) {
      setActiveFilter('proposals');
    }
  }, [recommendedAssetIds]);
  ...
  const filteredAssets = useMemo(() => {
    return assets.filter((asset) => {
      if (activeFilter === 'proposals') {
        return recommendedAssetIds ? recommendedAssetIds.includes(asset.id) : true;
      }
    ...
    <div className="w-full h-full relative">
      ...
      <div className="absolute top-4 sm:top-8 left-1/2 transform -translate-x-1/2 z-[1000] flex flex-nowrap items-center gap-1.5 sm:gap-2 bg-brand-deep/90 backdrop-blur-md px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-full border border-white/[0.08] shadow-lg pointer-events-auto max-w-[95vw] sm:max-w-max overflow-x-auto no-scrollbar">
        {recommendedAssetIds && recommendedAssetIds.length > 0 && (
          ...
          >
            <span>✨</span>
            <span>הצעות ({recommendedAssetIds.length})</span>
          </button>
        )}
        <button 
